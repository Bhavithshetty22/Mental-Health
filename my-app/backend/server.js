...
if (process.env.GOOGLE_APPLICATION_CREDENTIALS) {
  try {
    ...
    console.error(
      "❌ Could not instantiate Text-to-Speech client:",
      err?.message
    );
  }
}
...
console.log(
  "DEBUG HUGGING_FACE_TOKEN:",
  process.env.HUGGING_FACE_TOKEN ? "present" : "MISSING"
);
console.log(
  "DEBUG VERTEX_PROJECT_ID:",
  process.env.VERTEX_PROJECT_ID ? "present" : "MISSING"
);
console.log(
  "DEBUG VERTEX_LOCATION:",
  process.env.VERTEX_LOCATION || "us-central1 (default)"
);
console.log(
  "DEBUG IMAGEN_MODEL:",
  process.env.IMAGEN_MODEL || "imagen-3.0-generate-001 (default)"
);
console.log(
...
if (GEMINI_KEY) {
  try {
    ...
    console.error(
      "❌ Could not instantiate GoogleGenerativeAI client:",
      err?.message
    );
  }
}
...
function detectEmotionFallback(text) {
  ...
  const predictions = Object.keys(scores)
    .map((emotion) => ({
      label: emotion,
...
async function detectMoodWithFallback(text) {
  ...
  for (let i = 0; i < EMOTION_MODELS.length; i++) {
    ...
    try {
      console.log(
        `Trying model ${i + 1}/${EMOTION_MODELS.length}: ${modelUrl}`
      );
      ...
      if (Array.isArray(predictions) && predictions.length > 0) {
        ...
        predictions = predictions.map((pred) => ({
          label:
            pred.label?.toLowerCase() || pred.LABEL?.toLowerCase() || "unknown",
...
app.post("/api/detect-mood", async (req, res) => {
  try {
    ...
    console.log(
      `Detecting mood for text (${text.length} chars):`,
  ...
  try {
    ...
    if (!ttsClient) {
      return res.status(500).json({
        ...
        details:
          "Please ensure GOOGLE_APPLICATION_CREDENTIALS is set and Text-to-Speech API is enabled",
    ...
    if (error.code === 7) {
      return res.status(403).json({
        ...
        details:
          "Please ensure Text-to-Speech API is enabled and service account has proper permissions",
...
Output only the final artistic image prompt, nothing else.`,
    ...
    if (error.message?.includes("access token")) {
      return res.status(500).json({
        ...
        details:
          "Please check GOOGLE_APPLICATION_CREDENTIALS path and service account permissions",
        code: "AUTH_ERROR",
      });
    }
    ...
    if (error.response?.status === 403) {
      return res.status(403).json({
        ...
        details:
          "Please ensure Vertex AI API is enabled and service account has proper permissions",
        code: "PERMISSION_DENIED",
      });
    }
  ...
  try {
    ...
    if (detectCrisis(situation)) {
      ...
      return res.json({
        crisis: true,
        message:
          "I'm really sorry you're feeling this way. Please reach out for help immediately:",
        resources: [
...
— Future You`;
      ...
      if (process.env.MONGO_URI) {
        try {
          ...
          console.error(
            "Warning: failed to save fake letter:",
            saveErr?.message
          );
        }
      }
...
- End with hope and encouragement`;
    ...
    if (process.env.MONGO_URI) {
      try {
        ...
        console.error(
          "Warning: failed to save generated letter:",
          saveErr?.message
        );
      }
    }
